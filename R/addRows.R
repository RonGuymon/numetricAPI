#' @title Add rows to a dataset (V2)
...
#' @export
addRows <- function(apiKey, datasetId, dataframeName, chunkSize = 1500, printSample = F, nullArrays = F){
  ...
  for(i in 1:iterations){
    ...
    r <- POST(paste("https://api.numetric.com/v2/dataset/", datasetId, "/rows", sep = ""),
  ...
  s <- GET(paste("https://api.numetric.com/v2/dataset/", datasetId, "/index", sep = ""),
           add_headers("Authorization" = apiKey)
  )
  return(s$status)
}
